  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="mb-4">
          <p className="text-teal-500 text-sm">
            {activeCategory === 'all'
              ...
              : `Showing latest in ${NAV_CATEGORIES.find((cat) => cat.slug === activeCategory)?.label ?? activeCategory}`}
          </p>
        </div>
      )}
      ...
      {query && articles.length > 0 && (
        <div className="mb-4">
          <p className="text-teal-500 text-sm">{articles.length} result{articles.length !== 1 ? 's' : ''} for &ldquo;{query}&rdquo;</p>
        </div>
      )}
